...
class JSONLoader: NSObject {
    ...
    init(_ persistentContainer: NSPersistentContainer) {
        let aContext = persistentContainer.newBackgroundContext()
        aContext.mergePolicy = NSMergeByPropertyObjectTrumpMergePolicy
        // Set unused undoManager to nil for macOS (it is nil by default on iOS)
        // to reduce resource requirements.
        aContext.undoManager = nil
        self.context = aContext
        self.persistentContainer = persistentContainer
    }

    public func loadData(meetingID: NSManagedObjectID) async {
        let baseURL = URL(string: "https://datatracker.ietf.org")
        let meeting: Meeting? = context.object(with: meetingID) as? Meeting
        ...
        if let meeting = meeting {
                ...
                do {
                    ...
                    for obj in objs {
                        ...
                        case .location(let loc):
                            updateLocation(meeting:meeting, location:loc)
                        case .parent(let area):
                            updateArea(parent:area)
                        case .session(_):
                            continue
                        }
                    }
                    ...
                    for obj in objs {
                        ...
                        case .session(let JSONsession):
                            if let baseURL = baseURL {
                                updateSession(baseURL: baseURL, dayFormatter:dayFormatter, rangeFormatter:rangeFormatter, meeting:meeting, session:JSONsession)
                            }
                        }
                    }
            ...
            print("Unable to find meeting with ID: \(meetingID)")
        }
    }

    public func loadDrafts(groupID: NSManagedObjectID, limit: Int32, offset: Int32) async {
        let group: Group? = context.object(with: groupID) as? Group
        let url: URL
        ...
        if let group = group {
            ...
            do {
                ...
                do {
                    ...
                    for obj in json_docs.objects {
                        updateDocument(group:group, kind:.draft, document:obj)
                    }
                } catch DecodingError.dataCorrupted(let context) {
                    print(context)
                } catch DecodingError.keyNotFound(let key, let context) {
            ...
            print("loadDrafts: Unable to find group with ID: \(groupID)")
        }
    }
    ...
    public func loadRelatedDrafts(groupID: NSManagedObjectID, limit: Int32, offset: Int32) async {
        let group: Group? = context.object(with: groupID) as? Group
        let url: URL
        ...
        if let group = group {
                ...
                do {
                    ...
                    for obj in json_docs.objects {
                        if !obj.name.starts(with: "draft-ietf-\(group.acronym!)") {
                            updateDocument(group:group, kind:.related, document:obj)
                        }
                    }
                } catch DecodingError.dataCorrupted(let context) {
                    print(context)
            ...
            print("loadRelatedDrafts: Unable to find group with ID: \(groupID)")
        }
    }

    public func loadRecordingDocument(sessionID: NSManagedObjectID) async {
        let session: Session? = context.object(with: sessionID) as? Session
        if let session = session {
            ...
            print("loadRecordingDocument: Unable to find session with ID: \(sessionID)")
        }
    }

    public func loadCharterDocument(groupID: NSManagedObjectID) async {
        let group: Group? = context.object(with: groupID) as? Group
        ...
        if let group = group {
            ...
            do {
                ...
                do {
                    ...
                    updateDocument(group:group, kind:.charter, document:json_doc)
                } catch DecodingError.dataCorrupted(let context) {
                    print(context)
                } catch DecodingError.keyNotFound(let key, let context) {
                    print("Key '\(key)' not found:", context.debugDescription)
            ...
            print("loadCharterDocument: Unable to find group with ID: \(groupID)")
        }
    }

    private func updateDocument(group: Group, kind: DocumentKind, document: JSONDocument) {
    ...
    private func updateLocation(meeting:Meeting, location: JSONLocation) {
    ...
    private func updateArea(parent: Parent) {
    ...
    private func findArea(name: String) -> Area? {
        let area: Area?
    ...
    private func findLocation(meeting: Meeting, name: String) -> Location? {
    ...
    private func updateGroup(group: JSONGroup) -> Group? {
        let g: Group!
    ...
    private func updatePresentation(presentation: JSONPresentation) -> Presentation? {
        let p: Presentation!
    ...
    private func updateSession(baseURL: URL, dayFormatter: DateFormatter, rangeFormatter: DateFormatter, meeting: Meeting, session: JSONSession) {
        ...
        let group_obj = updateGroup(group:session.group)
        ...
        let loc = findLocation(meeting:meeting, name:session.location)
        if s.location != loc {
            s.location = loc
            save = true
        }
        ...
        if let new_json = session.presentations {
            for presentation in new_json {
                if let p = updatePresentation(presentation: presentation) {
                    new.insert(p)
                }
            }
        }
